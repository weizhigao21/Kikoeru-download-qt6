...
class TranslatorService:
    ...
    def __init__(self):
        ...
        self._system_prompt = ""  # 翻译 system prompt（完全自定义，来自设置；空 = 回退内置默认）
    ...
    def update_config(self, api_key: str, base_url: str, model: str, thinking_enabled: bool = True,
        ...
        Args:
            system_prompt: 翻译用的完整 system prompt（来自设置，完全自定义；
                           空字符串 = 回退内置默认规则 _SYSTEM_PROMPT，拆解请求不受影响）
        """
    ...
    def _request_translation(self, text: str, timeout: int, thinking: bool, mode: str = "translate") -> Optional[str]:
        ...
        if mode == "explain":
            ...
            # 翻译 system prompt 完全使用设置里的自定义提示词（v2.2.2 起不再拼接内置规则）；
            # 设置为空时回退内置默认规则保证基础可用
            system_prompt = self._system_prompt if self._system_prompt else self._SYSTEM_PROMPT
