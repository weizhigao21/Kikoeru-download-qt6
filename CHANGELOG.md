...
- **修复：翻译缓存导致改提示词后仍返回旧译文** — 此前缓存键仅 = 原文，修改提示词后同一标题命中旧缓存、新提示词不生效（"老是使用同一个"）。现缓存键 = **(原文, 当前 system prompt)**：改提示词后同一标题自动重新请求、不命中旧结果；`update_config` 检测提示词变化时自动清空内存翻译缓存；`get_cached` / `invalidate` 同步使用新键。拆解缓存不受影响（键仍为原文）

## v2.2.2

- **修复：翻译提示词被内置规则压制** — 此前翻译请求的 system prompt = 内置规则（强制"只输出译文"）+ 设置文本追加拼接；当用户在设置里填写自定义格式提示词（如"输出原文+译文+拆解"）时，内置规则与用户提示词冲突且压过用户要求。现改为：**设置文本即完整 System Prompt**，翻译请求不再拼接/追加任何内置规则，完全按用户提示词执行；设置为空时回退内置默认规则（`_SYSTEM_PROMPT`）保证基础可用。拆解请求不受影响

## v2.2.1

- **新增：翻译上下文自定义** — 设置 → AI 翻译页新增「翻译上下文 / 风格提示」多行文本框（默认预填推荐上下文：DLsite 音声标题术语惯例、保留 ♡～☆ 装饰符号、语气贴近原题、自造词音译、拉长音自然表达），翻译请求时注入 system prompt 提升翻译质量；清空 = 不注入；仅作用于标题翻译，词义拆解请求不受影响。配置键 `ai_translate_context`，`translator.update_config` 新增 `context` 参数
...
- **修复：「没有下载」页滚动位置进一步保护** — `set_works` 新增 `preserve_scroll` 显式保护：局部刷新时保存滚动比例、分 0/30/120ms 三次按新最大值还原，彻底对抗底部任务条重建挤压视口导致的滚动位置漂移（实测连续刷新最大偏差 8px，不再有跳变）

## v2.1.1

- **修复：「没有下载」页刷新置顶 / 详情跳转** — 下载完成、采集入库新数据、删除记录、隐藏作品等自动刷新不再把列表滚动条拉回顶部（`_show_works` 新增 `scroll_to_top` 参数按场景控制，实测 model 重建后滚动位置保持）；局部刷新同时保持当前详情面板不跳转（新增 `show_first_detail` 参数），翻页/切 tab 仍正常显示
- **优化：打包体积精简（159MB → 87MB）** — `音声浏览下载.spec` 排除 numpy（约 47MB OpenBLAS，项目未用）/ PyQt6.QtPdf；打包脚本自动删除软件 OpenGL 渲染器 opengl32sw.dll（20MB）与非简体中文 Qt 翻译（仅留 3 个 zh_CN .qm）；aria2（3.6MB）不再打包进 exe，需在 exe 旁放置 `aria2/` 目录或配置 `aria2_dir`
- **新增：一键打包脚本 `打包.bat`** — 自动定位 Python（py -3.10 降级链）、检查/安装依赖与 PyInstaller、备份旧产物、打包、精简产物、校验 exe；GBK 编码 + chcp 936 兼容中文路径
- **修复：README 环境依赖补充 zhconv**（繁简转换依赖此前遗漏）

## v2.1.0

- **新增：「没有下载」下载页 tab** — 新增第 4 个 tab，展示数据库中所有未下载的作品（其他语言版本复用卡片紫色可点击标签），按发售日期倒序（最新发售在前）；双击正常下载，卡片复用现有 delegate（已下载 ✓ / 版本标签 / 翻译缓存自动生效）
- **新增：后台自动采集** — 程序运行时后台每 3s 采集「最新收录」作品入本地库，头部追新（第 1 页有新作品优先入库）+ 游标补历史双轨调度；递增步长跳页快速跨历史区、回退锁定避免跳页漏边界；采集游标持久化到 `collector_state` 表（断点续扫、跨天计数归零但游标保持）；每日入库上限兜底不冲击服务器；设置页可开关 / 调上限 / 调头部追新间隔
- **数据层增强** — `works` 表新增 `release`（发售日期）列并自动迁移；新增 `upsert_works`（ON CONFLICT 去重写入，不覆盖 hidden/create_time，解决翻页采集的 work_id UNIQUE 冲突）、`existing_work_ids`（批量查重）、`get_all_works`（下载页数据源，按发售日期倒序）、`update_release`；新增采集状态读写 `save/load_collector_state`

## v2.0.9

- **新增：下载管理右键删除任务** — 「正在下载 / 已完成」列表支持右键菜单删除任务：下载中/排队任务显示「取消并删除任务」（先取消下载再移除记录），已完成/失败任务显示「删除任务」；仅移除任务记录，不删除磁盘上的已下载文件；删除前弹确认框。底层新增 `DownloadManager.remove_task`
- **修复：下载完成后繁简转换偶发不生效** — 下载完成后的字幕/繁简转换运行在 daemon 线程，程序退出（关闭窗口/崩溃）时会立即被强杀，转换中断导致作品文件名与字幕内容保持繁体且无兜底。现：① 后处理线程改为非 daemon，主窗口关闭时等待转换完成（最多 15s）后才退出；② 下载管理器「已完成」页新增「繁简转换」按钮，可对已完成任务目录手动重新执行转换，解决存量繁体文件
- **增强：繁简转换可靠性** — 后处理线程登记跟踪（`_postprocess_threads`），`wait_for_postprocess` 供退出前等待；`reprocess_t2s` 提供手动重转入口（跳过进行中任务，目录去重）

## v2.0.8

- **新增：下载目录名标题长度可配置** — 原 `WorkDownloader._get_save_dir` 硬编码截断标题到 50 字符（长标题作品文件夹名被截断，README/设置均无说明）；现改为设置项「目录名标题最大长度」（默认 120，0-500，0=不限制，`config.json` 键 `folder_title_max_len`），保存后立即生效
- **增强：目录名过滤健壮性** — 同一函数补强：① 过滤控制字符与尾部空格/句点（Windows 会自动去除，造成意外截断）；② Windows 保留设备名兜底（CON/PRN/AUX/NUL/COM1-9/LPT1-9 加 `_` 前缀避免创建失败）；③ Windows 路径长度保护（完整路径超预算时按需收缩标题，根目录越深预算越小，预留子文件夹与文件名空间）
