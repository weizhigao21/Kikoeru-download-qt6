...
chcp 936 >nul
rem ============================================
rem  音声浏览下载 一键打包脚本 (PyInstaller onedir)
...
%PYCMD% -c "import PyQt6, requests, PIL, zhconv" >nul 2>nul
if errorlevel 1 (
    echo [2/5] 安装运行依赖 ...
    %PYCMD% -m pip install PyQt6 requests Pillow zhconv
...
rem 避免 PyInstaller COLLECT 阶段对已存在的 dist\音声浏览下载 执行 rmtree 触发删除保护
if exist "dist\音声浏览下载" move "dist\音声浏览下载" "dist\old_%RANDOM%%RANDOM%" >nul
rem PyInstaller 二进制缓存默认写 LOCALAPPDATA，改为项目内（自包含，避免权限问题）
set "PYINSTALLER_CONFIG_DIR=%CD%\build\pyinstaller-cache"
%PYCMD% -m PyInstaller 音声浏览下载.spec --noconfirm
if errorlevel 1 goto :fail

rem ---- 6. 验证产物并清理旧备份 ----
set "OUT=dist\音声浏览下载\音声浏览下载.exe"
if not exist "%OUT%" (
    echo [错误] 未找到产物 %OUT%
    goto :fail
)
...
del /q "dist\音声浏览下载\_internal\PyQt6\Qt6\bin\opengl32sw.dll" 2>nul
for %%f in ("dist\音声浏览下载\_internal\PyQt6\Qt6\translations\*.qm") do (
...
echo ============================================
echo.
echo 产物目录: %~dp0dist\音声浏览下载\
echo 主程序:   %~dp0dist\音声浏览下载\音声浏览下载.exe
echo.
echo 整个 dist\音声浏览下载 目录即完整程序，可整体压缩为 zip 分发。
echo 运行时数据（settings\：config.json、works.db、download_history.db）
echo 位于 exe 同级，首次运行自动创建。
echo.
explorer "%~dp0dist\音声浏览下载" >nul 2>nul
pause
exit /b 0
